...
<body class="bg-white text-gray-800 p-8">
    ...
    <div class="flex justify-center items-center mb-6">
        ...
        <a href="./createUserAccountPage.php" class="bg-green-200 hover:bg-green-300 text-sm px-4 py-2 rounded-md shadow inline-block ml-4">
            Create User Account
        </a>
    </div>
                ...
                <?php if (!empty($userAccounts)): ?>
                    <?php foreach ($userAccounts as $account): ?>
                        <tr class="<?php echo ($account['userAccountID'] % 2 == 0) ? 'bg-white' : ''; ?>">
                            ...
                            <td class="px-4 py-2 border"><?php echo htmlspecialchars($account['userProfileName']); ?></td>
                            <td class="px-4 py-2 border"><?php echo $account['status'] ? 'Active' : 'Suspended'; ?></td>
                            <td class="px-4 py-2 border"><a href="viewUserAccountPage.php?id=<?php echo $account['userAccountID']; ?>" class="text-blue-500 hover:underline">View</a></td>
                            <td class="px-4 py-2 border"><a href="updateUserAccountPage.php?id=<?php echo $account['userAccountID']; ?>" class="text-green-500 hover:underline">Update</a></td>
                            <td class="px-4 py-2 border">
                                <?php if ($account['status']): ?>
                                    <form action="viewAllUserAccountsPage.php" method="post" onsubmit="return confirm('Are you sure you want to suspend this user account?');">
                ...
                success: function(response) {
                    ...
                    $.each(response, function(index, account) {
                        ...
                        var row = '<tr class="' + bgClass + '">' +
                            ...
                            '<td class="px-4 py-2 border">' + account.userProfileName + '</td>' +
                            '<td class="px-4 py-2 border">' + statusText + '</td>' +
                            '<td class="px-4 py-2 border"><a href="viewUserAccountPage.php?id=' + account.userAccountID + '" class="text-blue-500 hover:underline">View</a></td>' +
                            '<td class="px-4 py-2 border"><a href="updateUserAccountPage.php?id=' + account.userAccountID + '" class="text-green-500 hover:underline">Update</a></td>' +
